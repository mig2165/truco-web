...
async function runTest(gameIndex) {
    return new Promise((resolve, reject) => {
        ...
        p1 = (0, socket_io_client_1.io)(SERVER_URL);
        p1.on('connect', () => {
            // Create room
            p1.emit('createRoom', 'Bot1', (resId) => {
                roomId = resId;
                p1.emit('joinRoom', roomId, 'Bot1', 1);
                p2 = (0, socket_io_client_1.io)(SERVER_URL);
                p2.on('connect', () => { p2.emit('joinRoom', roomId, 'Bot2', 2); });
                p3 = (0, socket_io_client_1.io)(SERVER_URL);
                p3.on('connect', () => { p3.emit('joinRoom', roomId, 'Bot3', 1); });
                p4 = (0, socket_io_client_1.io)(SERVER_URL);
                p4.on('connect', () => {
                ...
                const takeAction = () => {
                    ...
                    if (!activeBot || currentState.status !== 'playing')
                        return;
                    // if mao de onze
                    ...
                    if (currentState._phase === 'WAITING_FOR_HAND_PHASE') {
                        const me = currentState.players.find((p) => p.id === activeBot.id);
                    ...
                    if (currentState._phase === 'TRICK_PHASE') {
                        ...
                        const validCardIndex = hand.findIndex((c) => c !== null);
                        if (validCardIndex !== -1) {
